...
| [04-finish-vault-migration.md](./04-finish-vault-migration.md) | full ship | 5 | Vault deployed via the Sprint 4 Ansible role; `kubinate-vault-migrate` binary + dry-run + idempotency; production cutover via the Sprint 4 runbook stub (now filled in); `KUBINATE_KEK` retired from CI. |
